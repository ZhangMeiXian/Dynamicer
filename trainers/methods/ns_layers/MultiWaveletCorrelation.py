...
from trainers.methods.ns_layers.utils import get_filter
...
class MultiWaveletTransform(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask=None, tau=None, delta=None):
        ...
        tau = tau.permute(0, 3, 1, 2) if not isinstance(tau, float) else tau
        delta = delta.permute(0, 3, 1, 2) if not isinstance(delta, float) else delta
...
class MultiWaveletCross(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, tau=None, delta=None):
        ...
        for i in range(ns - self.L):
            ...
            dq, sq = Ud_q[i], Us_q[i]
            dv, sv = Ud_v[i], Us_v[i]
            Ud += [self.attn1(dq[0], dk[0], dv[0], mask, tau=tau, delta=delta)[0] +
                   self.attn2(dq[1], dk[1], dv[1], mask, tau=tau, delta=delta)[0]]
            Us += [self.attn3(sq, sk, sv, mask, tau=tau, delta=delta)[0]]
...
class sparseKernelFT1d(nn.Module):
    """
    sparse kernel for fourier transform
    """
